  ...
  try {
    ...
    for (let i = 0; i < items.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(item => {
        ...
        return sql`
          INSERT INTO rss_items (id, title, url, source, sid, published_at)
          VALUES (${id}, ${item.title}, ${item.url}, ${item.source}, ${item.sid}, ${item.published_at})
...
export async function getRssItems(limit = 100) {
  ...
  try {
    const { rows } = await sql`
      SELECT id, title, url, source, sid, published_at as "published_at", created_at as "created_at"
      FROM rss_items
