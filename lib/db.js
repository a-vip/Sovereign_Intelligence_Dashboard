...
export async function getEvents(timespan = '24h') {
  ...
  try {
    ...
    let rows = [];
    if (timespan === 'today') {
      const res = await sql`SELECT * FROM sigint_events WHERE COALESCE(original_post_time, timestamp) >= CURRENT_DATE - INTERVAL '14 days' AND status != 'draft' ORDER BY COALESCE(original_post_time, timestamp) DESC`;
      rows = res.rows;
    } else {
      const res = await sql`SELECT * FROM sigint_events WHERE COALESCE(original_post_time, timestamp) >= NOW() - INTERVAL '14 days' AND status != 'draft' ORDER BY COALESCE(original_post_time, timestamp) DESC LIMIT 500`;
      rows = res.rows;
    }
