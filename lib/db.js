...
export async function archiveEvent(eventId, archivedBy) {
  if (!process.env.POSTGRES_URL) {
    ...
    archive.push({ ...event, source_table: 'sigint_events', archived_by: archivedBy, archived_at: new Date().toISOString(), original_created_at: event.created_at, edited: event.edited || false });
...
export async function archiveRssItem(itemId, archivedBy) {
  if (!process.env.POSTGRES_URL) {
    ...
    archive.push({
      ...
      original_created_at: item.created_at, edited: item.edited || false
    });
    writeJsonFile(archivePath, archive);
    return item;
  }
