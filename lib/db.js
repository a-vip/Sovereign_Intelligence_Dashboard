...
export async function updateEvent(eventId, fields) {
  ...
  try {
    const checkRes = await sql`SELECT id FROM sigint_events WHERE id = ${eventId}`;
    if (checkRes.rows.length === 0) {
      ...
      await sql`
        ...
        VALUES (${eventId}, ${title}, ${category}, ${severity}, ${location}, ${lat}, ${lon}, ${timestamp}, ${url}, ${JSON.stringify(details)}, TRUE)
      `;
    } else {
      ...
      await sql`UPDATE sigint_events SET edited = TRUE WHERE id = ${eventId}`;
    }
...
export async function updateRssItem(itemId, fields) {
  ...
  try {
    const checkRes = await sql`SELECT id FROM rss_items WHERE id = ${itemId}`;
    if (checkRes.rows.length === 0) {
      ...
      await sql`
        ...
        VALUES (${itemId}, ${title}, ${url}, ${source}, ${sid}, ${location}, ${latVal}, ${lonVal}, ${category}, ${severity}, ${summary}, ${publishedAt}, TRUE)
      `;
    } else {
      ...
      await sql`UPDATE rss_items SET edited = TRUE WHERE id = ${itemId}`;
    }
...
export async function archiveEvent(eventId, archivedBy) {
  if (!process.env.POSTGRES_URL) {
    ...
    if (idx !== -1) {
      ...
      const rIdx = regs.findIndex(r => r.id === eventId);
      if (rIdx !== -1) {
        const reg = regs[rIdx];
        event = {
          id: reg.id,
        ...
        const rssIdx = rssItems.findIndex(i => i.id === eventId);
        if (rssIdx !== -1) {
        ...
        const sIdx = staticEvents.findIndex(e => e.id === eventId);
        if (sIdx !== -1) {
          ...
          const sug = suggestions.find(s => s.id === eventId || s.target_id === eventId);
          if (sug) {
            event = {
              id: eventId,
    ...
    if (!archive.some(e => e.id === eventId)) {
      archive.push({
        ...event,
  ...
  try {
    ...
    const resSig = await sql`SELECT id FROM sigint_events WHERE id = ${eventId}`;
    ...
    const resReg = await sql`SELECT id, title, jurisdiction, status, area, date, description, lat, lon, created_at FROM ai_regulations WHERE id = ${eventId}`;
    if (resReg.rows.length > 0) {
      ...
      `;
      await sql`DELETE FROM ai_regulations WHERE id = ${eventId}`;
      return { id: eventId, archived: true };
    }

    // Copy/Delete from rss_items
    const resRss = await sql`SELECT id FROM rss_items WHERE id = ${eventId}`;
    if (resRss.rows.length > 0) {
      await sql`
        ...
        FROM rss_items WHERE id = ${eventId}
      `;
      await sql`DELETE FROM rss_items WHERE id = ${eventId}`;
      return { id: eventId, archived: true };
    }
