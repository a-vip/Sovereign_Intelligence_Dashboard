...
export async function initDb() {
  try {
    await sql`
      ...
      CREATE INDEX IF NOT EXISTS idx_sigint_timestamp ON sigint_events (timestamp DESC);
      CREATE INDEX IF NOT EXISTS idx_sigint_severity ON sigint_events (severity);
      CREATE INDEX IF NOT EXISTS idx_sigint_category ON sigint_events (category);
      CREATE INDEX IF NOT EXISTS idx_vault_modified ON vault_docs (last_modified DESC);
    `;
...
export async function saveEvents(events) {
  ...
  try {
    // Process in smaller batches to avoid Vercel Postgres query limits
    const BATCH_SIZE = 25;
    ...
    console.error('Database batch save error:', error);
  }
}
...
export async function saveVaultDocs(docs) {
  ...
  try {
    ...
    console.error('Vault docs save error:', error);
  }
}
...
export async function getVaultDocs() {
  try {
    ...
    console.error('Vault docs fetch error:', error);
    return [];
  }
}
