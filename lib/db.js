...
export async function initDb() {
  ...
  try {
    ...
    } catch (e) { console.warn('Migration warning (rss_items.location):', e.message); }
    try {
      await sql`ALTER TABLE rss_items ADD COLUMN IF NOT EXISTS latitude FLOAT`;
    } catch (e) { console.warn('Migration warning (rss_items.latitude):', e.message); }
    try {
      await sql`ALTER TABLE rss_items ADD COLUMN IF NOT EXISTS longitude FLOAT`;
    } catch (e) { console.warn('Migration warning (rss_items.longitude):', e.message); }
    try {
      await sql`ALTER TABLE rss_items ADD COLUMN IF NOT EXISTS category TEXT`;
    } catch (e) { console.warn('Migration warning (rss_items.category):', e.message); }
    try {
      await sql`ALTER TABLE rss_items ADD COLUMN IF NOT EXISTS severity INTEGER DEFAULT 1`;
    } catch (e) { console.warn('Migration warning (rss_items.severity):', e.message); }
    try {
      await sql`ALTER TABLE rss_items ADD COLUMN IF NOT EXISTS summary TEXT`;
    } catch (e) { console.warn('Migration warning (rss_items.summary):', e.message); }
    try {
      await sql`ALTER TABLE suggestions ADD COLUMN IF NOT EXISTS screenshot TEXT`;
    } catch (e) { console.warn('Migration warning (suggestions.screenshot):', e.message); }
    try {
...
export async function saveEvents(events) {
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    sanitized.forEach(e => {
      ...
      const isEdited = existing.edited === true || existing.edited === 'true';
      eventMap.set(e.id, {
        ...existing,
        ...e,
        title: isEdited ? existing.title : e.title,
        category: isEdited ? existing.category : e.category,
        severity: isEdited ? existing.severity : e.severity,
...
export async function getEvents(timespan = '24h') {
  ...
  try {
    ...
    let rows = [];
    if (timespan === 'today') {
      const res = await sql`SELECT * FROM sigint_events WHERE timestamp >= CURRENT_DATE - INTERVAL '7 days' ORDER BY timestamp DESC`;
      rows = res.rows;
    } else {
      const res = await sql`SELECT * FROM sigint_events WHERE timestamp >= NOW() - CAST(${interval} AS INTERVAL) - INTERVAL '7 days' ORDER BY timestamp DESC LIMIT 500`;
      rows = res.rows;
    }
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    filtered.forEach(item => {
      ...
      if (!existing) {
        ...
        existing.location = isEdited ? existing.location : (item.location || existing.location);
        existing.latitude = isEdited ? existing.latitude : (item.latitude !== undefined && item.latitude !== null ? item.latitude : existing.latitude);
        existing.longitude = isEdited ? existing.longitude : (item.longitude !== undefined && item.longitude !== null ? item.longitude : existing.longitude);
  ...
  try {
    ...
    if (checkRes.rows.length === 0) {
      ...
      await sql`
        ...
        VALUES (${cleanEventId}, ${title}, ${category}, ${severity}, ${location}, ${lat}, ${lon}, ${timestamp}, ${url}, ${JSON.stringify(details)}, TRUE)
      `;
    } else {
...
export async function archiveEvent(eventId, archivedBy) {
  ...
  if (!process.env.POSTGRES_URL) {
    const events = readJsonFile(LOCAL_EVENTS_FILE);
    const idx = events.findIndex(e => e.id === cleanEventId);
    let event;
    if (idx !== -1) {
      event = events.splice(idx, 1)[0];
      writeJsonFile(LOCAL_EVENTS_FILE, events);
    } else {
      // 1. Try to find it in AI regulations
      const regs = readJsonFile(LOCAL_AI_REGULATIONS_FILE);
      const rIdx = regs.findIndex(r => r.id === cleanEventId);
      if (rIdx !== -1) {
        const reg = regs[rIdx];
        event = {
          id: reg.id,
          title: reg.title || 'AI Regulation',
          category: reg.area || 'Political',
          severity: 2,
          location: reg.jurisdiction || 'Global',
          lat: reg.lat,
          lon: reg.lon,
          timestamp: reg.date || new Date().toISOString(),
          url: '',
          details: { description: reg.description || '', status: reg.status || '' }
        };
        regs.splice(rIdx, 1);
        writeJsonFile(LOCAL_AI_REGULATIONS_FILE, regs);
      } else {
        // 1.5 Try to find it in RSS items
        const rssItems = readJsonFile(LOCAL_RSS_FILE);
        const rssIdx = rssItems.findIndex(i => i.id === cleanEventId);
        if (rssIdx !== -1) {
          const item = rssItems.splice(rssIdx, 1)[0];
          event = {
            id: item.id,
            title: item.title || 'RSS Feed Signal',
            ...
            url: item.url,
            details: { summary: item.summary, source: item.source }
          };
          writeJsonFile(LOCAL_RSS_FILE, rssItems);
        } else {
        // 2. Try to find it in public/data/events.json (static events)
        ...
        try {
          ...
          console.error('Error loading static events during archive:', err);
        }
        const sIdx = staticEvents.findIndex(e => e.id === cleanEventId);
        if (sIdx !== -1) {
          event = staticEvents[sIdx];
        } else {
          // 3. Check if it is a local suggestion or feedback
          const suggestions = readJsonFile(LOCAL_SUGGESTIONS_FILE);
          const sug = suggestions.find(s => s.id === cleanEventId || s.target_id === cleanEventId);
          if (sug) {
            event = {
              id: cleanEventId,
              title: sug.subject || 'Suggested Threat Marker',
              category: sug.category || 'Political',
              severity: parseInt(sug.severity) || 3,
              location: sug.location || 'Global',
              lat: 0, lon: 0,
              timestamp: new Date().toISOString(),
              url: sug.url || '',
              details: sug.details
    ...
    if (!archive.some(e => e.id === cleanEventId)) {
      archive.push({
        ...event,
        source_table: 'sigint_events',
        archived_by: archivedBy,
        archived_at: new Date().toISOString(),
        original_created_at: event.created_at || event.timestamp || new Date().toISOString(),
  ...
  try {
    // Copy/Delete from sigint_events
    const resSig = await sql`SELECT id FROM sigint_events WHERE id = ${cleanEventId}`;
    if (resSig.rows.length > 0) {
      await sql`
        ...
        SELECT id, title, category, severity, location, lat, lon, timestamp, url, details, 'sigint_events', ${archivedBy}, created_at, edited
        FROM sigint_events WHERE id = ${cleanEventId}
      `;
      await sql`DELETE FROM sigint_events WHERE id = ${cleanEventId}`;
      return { id: cleanEventId, archived: true };
    }

    // Copy/Delete from ai_regulations
    const resReg = await sql`SELECT id, title, jurisdiction, status, area, date, description, lat, lon, created_at FROM ai_regulations WHERE id = ${cleanEventId}`;
    if (resReg.rows.length > 0) {
      ...
      await sql`
        ...
        VALUES (
          ...
          CURRENT_TIMESTAMP,
          '',
          ${JSON.stringify({ description: reg.description, status: reg.status, date: reg.date })},
          'ai_regulations',
          ${archivedBy},
          ${reg.created_at || 'CURRENT_TIMESTAMP'},
          FALSE
        )
      `;
      await sql`DELETE FROM ai_regulations WHERE id = ${cleanEventId}`;
      return { id: cleanEventId, archived: true };
    }

    // Copy/Delete from rss_items
    const resRss = await sql`SELECT id FROM rss_items WHERE id = ${cleanEventId}`;
    if (resRss.rows.length > 0) {
      await sql`
        ...
        SELECT id, title, category, severity, location, latitude, longitude, published_at, url,
          ...
          'rss_items', ${archivedBy}, created_at, edited
        FROM rss_items WHERE id = ${cleanEventId}
      `;
      await sql`DELETE FROM rss_items WHERE id = ${cleanEventId}`;
      return { id: cleanEventId, archived: true };
    }

    // Try to find it in public/data/events.json (static events)
    const staticPath = path.resolve('public', 'data', 'events.json');
    let staticEvents = [];
    try {
      if (fs.existsSync(staticPath)) {
      ...
      console.error('Error loading static events in Postgres mode during archive:', err);
    }
    const staticEvent = staticEvents.find(e => e.id === cleanEventId);
    if (staticEvent) {
      await sql`
        INSERT INTO archived_events (id, title, category, severity, location, lat, lon, timestamp, url, details, source_table, archived_by, original_created_at, edited)
        VALUES (
          ${staticEvent.id},
          ${staticEvent.title},
          ${staticEvent.category || 'Conflict'},
          ${staticEvent.severity || 1},
          ${staticEvent.location || 'Global'},
          ${staticEvent.lat || 0.0},
          ${staticEvent.lon || 0.0},
          CURRENT_TIMESTAMP,
          ${staticEvent.url || ''},
          ${JSON.stringify(staticEvent.details || {})},
          'static_events',
          ${archivedBy},
          CURRENT_TIMESTAMP,
          FALSE
...
export async function archiveRssItem(itemId, archivedBy) {
  ...
  if (!process.env.POSTGRES_URL) {
    const items = readJsonFile(LOCAL_RSS_FILE);
    const idx = items.findIndex(i => i.id === cleanItemId);
    let item;
    if (idx !== -1) {
      ...
      item = {
        id: cleanItemId,
        title: 'Archived RSS Feed Signal',
        category: 'Conflict',
        severity: 1,
        location: 'Global',
    ...
    const archivePath = path.resolve('archived-events-local.json');
    const archive = readJsonFile(archivePath);
    if (!archive.some(e => e.id === cleanItemId)) {
      archive.push({
        id: item.id,
        title: item.title,
        ...
        archived_by: archivedBy,
        archived_at: new Date().toISOString(),
        original_created_at: item.created_at || new Date().toISOString(),
  ...
  try {
    await sql`
      ...
      SELECT id, title, category, severity, location, latitude, longitude, published_at, url,
        jsonb_build_object('summary', COALESCE(summary, ''), 'source', COALESCE(source, '')),
        'rss_items', ${archivedBy}, created_at, edited
      FROM rss_items WHERE id = ${cleanItemId}
    `;
    await sql`DELETE FROM rss_items WHERE id = ${cleanItemId}`;
    return { id: cleanItemId, archived: true };
  } catch (error) {
...
export async function getArchivedIds() {
  if (!process.env.POSTGRES_URL) {
    const archivePath = path.resolve('archived-events-local.json');
    const archive = readJsonFile(archivePath);
    return new Set(archive.map(e => e.id));
  }
  try {
    const { rows } = await sql`SELECT id FROM archived_events`;
...
export async function getArchivedInfo() {
  ...
  archive.forEach(e => {
    ...
    if (e.url) {
      const urlNorm = e.url.split('?')[0].trim().toLowerCase();
