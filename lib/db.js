...
export async function initDb() {
    ...
    if (!fs.existsSync(LOCAL_EVENTS_FILE)) {
      ...
      localEvents.forEach(e => {
        ...
        if (e.original_post_time === undefined) {
          e.original_post_time = e.timestamp || new Date().toISOString();
          updated = true;
        }
    ...
    const localArchivePath = path.resolve('archived-events-local.json');
    if (fs.existsSync(localArchivePath)) {
      const localArchive = readJsonFile(localArchivePath);
      let archiveUpdated = false;
      localArchive.forEach(e => {
        if (e.original_post_time === undefined) {
          e.original_post_time = e.timestamp || new Date().toISOString();
          archiveUpdated = true;
        }
      ...
      if (archiveUpdated) writeJsonFile(localArchivePath, localArchive);
    }
  ...
  dbInitPromise = (async () => {
    try {
      ...
      await sql`CREATE TABLE IF NOT EXISTS sigint_events (
        ...
        original_post_time TIMESTAMP WITH TIME ZONE,
      ...
      // Authentic Timestamps & Metadata separation migrations
      try {
        await sql`ALTER TABLE sigint_events ADD COLUMN IF NOT EXISTS original_post_time TIMESTAMP WITH TIME ZONE`;
      } catch (e) { console.warn('Migration warning (sigint_events.original_post_time):', e.message); }
      try {
        await sql`ALTER TABLE sigint_events ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP`;
      } catch (e) { console.warn('Migration warning (sigint_events.updated_at):', e.message); }
      try {
        await sql`ALTER TABLE sigint_events ADD COLUMN IF NOT EXISTS last_edited_by TEXT`;
      } catch (e) { console.warn('Migration warning (sigint_events.last_edited_by):', e.message); }
      try {
        await sql`UPDATE sigint_events SET original_post_time = timestamp WHERE original_post_time IS NULL`;
      } catch (e) { console.warn('Migration warning (sigint_events backfill):', e.message); }

      try {
        await sql`ALTER TABLE archived_events ADD COLUMN IF NOT EXISTS original_post_time TIMESTAMP WITH TIME ZONE`;
      } catch (e) { console.warn('Migration warning (archived_events.original_post_time):', e.message); }
      try {
        await sql`ALTER TABLE archived_events ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE`;
      } catch (e) { console.warn('Migration warning (archived_events.updated_at):', e.message); }
      try {
        await sql`ALTER TABLE archived_events ADD COLUMN IF NOT EXISTS last_edited_by TEXT`;
      } catch (e) { console.warn('Migration warning (archived_events.last_edited_by):', e.message); }
      try {
        await sql`UPDATE archived_events SET original_post_time = timestamp WHERE original_post_time IS NULL`;
      } catch (e) { console.warn('Migration warning (archived_events backfill):', e.message); }
...
export async function saveEvents(events) {
    ...
    sanitized.forEach(e => {
      ...
      if (!isEdited) {
        ...
        if (matched) {
          ...
          existing.original_post_time = matched.original_post_time;
        }
      }
      ...
      eventMap.set(e.id, {
        ...
        original_post_time: existing.original_post_time || e.original_post_time || e.timestamp || new Date().toISOString()
      });
    });
  ...
  try {
    ...
    const { rows: editedEvents } = await sql`
      SELECT title, url, location, lat, lon, category, severity, details, edited, original_post_time
      FROM sigint_events 
      WHERE edited = TRUE
    `;
    ...
    sanitized.forEach(event => {
      ...
      if (matchedEdited) {
        ...
        event.original_post_time = matchedEdited.original_post_time;
      }
    });
    ...
    for (let i = 0; i < sanitized.length; i += BATCH_SIZE) {
      ...
      await Promise.all(batch.map(event => 
        sql`
          INSERT INTO sigint_events (id, title, category, severity, location, lat, lon, timestamp, url, details, edited, original_post_time)
          VALUES (
            ${event.id}, 
            ${event.title}, 
            ${event.category}, 
            ...
            ${event.edited || false},
            ${event.original_post_time || event.timestamp}
          )
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    const sanitizedLocal = localEvents.map(e => {
      ...
      if (originalTs !== sanitized.timestamp) {
        hasChanges = true;
        sanitized.original_post_time = sanitized.timestamp;
      }
      return sanitized;
    });
    if (hasChanges) {
    ...
    // Sort local events by original_post_time descending, filtering out drafts for public dashboard
    return sanitizedLocal
      .filter(e => e.status !== 'draft')
      .sort((a, b) => new Date(b.original_post_time || b.timestamp) - new Date(a.original_post_time || a.timestamp))
      .slice(0, 500);
  }
  try {
    ...
    let rows = [];
    if (timespan === 'today') {
      const res = await sql`SELECT * FROM sigint_events WHERE (edited = TRUE OR (COALESCE(original_post_time, timestamp) >= CURRENT_DATE - INTERVAL '7 days')) AND status != 'draft' ORDER BY COALESCE(original_post_time, timestamp) DESC`;
      rows = res.rows;
    } else {
      const res = await sql`SELECT * FROM sigint_events WHERE (edited = TRUE OR (COALESCE(original_post_time, timestamp) >= NOW() - CAST(${interval} AS INTERVAL) - INTERVAL '7 days')) AND status != 'draft' ORDER BY COALESCE(original_post_time, timestamp) DESC LIMIT 500`;
      rows = res.rows;
    }
    ...
    const sanitizedRows = rows.map(r => {
      ...
      if (originalTs && sanitized.timestamp && new Date(originalTs).getTime() !== new Date(sanitized.timestamp).getTime()) {
        sql`UPDATE sigint_events SET timestamp = ${sanitized.timestamp}, original_post_time = ${sanitized.timestamp} WHERE id = ${sanitized.id}`.catch(err => {
...
export async function getAllEvents(page = 1, limit = 50, search = '') {
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    events.sort((a, b) => new Date(b.original_post_time || b.timestamp) - new Date(a.original_post_time || a.timestamp));
  ...
  try {
    ...
    if (search) {
      ...
      dataResult = await sql`SELECT * FROM sigint_events WHERE title ILIKE ${searchPattern} ORDER BY COALESCE(original_post_time, timestamp) DESC LIMIT ${limit} OFFSET ${offset}`;
    } else {
      countResult = await sql`SELECT count(*) as total FROM sigint_events`;
      dataResult = await sql`SELECT * FROM sigint_events ORDER BY COALESCE(original_post_time, timestamp) DESC LIMIT ${limit} OFFSET ${offset}`;
    }
    return {
...
export async function updateEvent(eventId, fields) {
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    if (idx === -1) {
      updatedEvent = {
        ...
        original_post_time: fields.original_post_time || fields.timestamp || new Date().toISOString(),
        url: fields.url || '',
      ...
      delete updatedFields.original_post_time;
      
      Object.assign(events[idx], updatedFields, {
        edited: true,
        updated_at: new Date().toISOString(),
        original_post_time: prevEvent.original_post_time || prevEvent.timestamp || new Date().toISOString()
      });
  ...
  try {
    ...
    if (checkRes.rows.length === 0) {
      ...
      await sql`
        INSERT INTO sigint_events (id, title, category, severity, location, lat, lon, timestamp, url, details, edited, status, original_post_time, updated_at)
        VALUES (${cleanEventId}, ${title}, ${category}, ${severity}, ${location}, ${lat}, ${lon}, ${timestamp}, ${url}, ${JSON.stringify(details)}, TRUE, ${status}, ${timestamp}, CURRENT_TIMESTAMP)
      `;
    } else {
      ...
      await sql`UPDATE sigint_events SET edited = TRUE, updated_at = CURRENT_TIMESTAMP WHERE id = ${cleanEventId}`;
      if (fields.last_edited_by !== undefined) {
        await sql`UPDATE sigint_events SET last_edited_by = ${fields.last_edited_by} WHERE id = ${cleanEventId}`;
      }
    }
      ...
      if (e.id === cleanId || matchTitle || matchUrl) {
        if (!archive.some(a => a.id === e.id)) {
          archive.push({
            ...
            last_edited_by: e.last_edited_by || null
          });
        }
      } else {
      ...
      if (r.id === cleanId || matchTitle || matchUrl) {
        if (!archive.some(a => a.id === r.id)) {
          archive.push({
            ...
            original_post_time: r.published_at || new Date().toISOString(),
            updated_at: null,
            last_edited_by: null
          });
        }
  ...
  try {
    ...
    for (const ev of sigRes.rows) {
      ...
      if (ev.id === cleanId || matchTitle || matchUrl) {
        await sql`
          INSERT INTO archived_events (id, title, category, severity, location, lat, lon, timestamp, url, details, source_table, archived_by, original_created_at, edited, original_post_time, updated_at, last_edited_by)
          VALUES (${ev.id}, ${ev.title}, ${ev.category}, ${ev.severity}, ${ev.location}, ${ev.lat}, ${ev.lon}, ${ev.timestamp}, ${ev.url}, ${JSON.stringify(ev.details)}, 'sigint_events', ${archivedBy}, ${ev.created_at || 'CURRENT_TIMESTAMP'}, ${ev.edited}, ${ev.original_post_time || ev.timestamp}, ${ev.updated_at || null}, ${ev.last_edited_by || null})
    ...
    for (const item of rssRes.rows) {
      ...
      if (item.id === cleanId || matchTitle || matchUrl) {
        await sql`
          INSERT INTO archived_events (id, title, category, severity, location, lat, lon, timestamp, url, details, source_table, archived_by, original_created_at, edited, original_post_time)
          VALUES (
            ${item.id}, ${item.title}, ${item.category}, ${item.severity}, ${item.location}, ${item.latitude}, ${item.longitude}, ${item.published_at}, ${item.url}, 
            ${JSON.stringify({summary: item.summary, source: item.source})}, 'rss_items', ${archivedBy}, ${item.created_at || 'CURRENT_TIMESTAMP'}, ${item.edited}, ${item.published_at}
          )
          ON CONFLICT (id) DO NOTHING
        `;
    ...
    if (resReg.rows.length > 0) {
      ...
      await sql`
        ...
        VALUES (
          ...
          ${JSON.stringify({ description: reg.description, status: reg.status, date: reg.date })}, 'ai_regulations', ${archivedBy}, ${reg.created_at || 'CURRENT_TIMESTAMP'}, FALSE, CURRENT_TIMESTAMP
        )
        ON CONFLICT (id) DO NOTHING
      `;
    ...
    if (finalCheck.rows.length === 0) {
      await sql`
        INSERT INTO archived_events (id, title, category, severity, location, lat, lon, timestamp, url, details, source_table, archived_by, original_created_at, edited, original_post_time)
        VALUES (${cleanId}, 'Archived Signal', 'Conflict', 1, 'Global', 0.0, 0.0, CURRENT_TIMESTAMP, '', '{}'::jsonb, 'sigint_events', ${archivedBy}, CURRENT_TIMESTAMP, FALSE, CURRENT_TIMESTAMP)
        ON CONFLICT (id) DO NOTHING
      `;
    }
...
export async function restoreArchivedEvent(archiveId) {
  if (!process.env.POSTGRES_URL) {
    ...
    if (event.source_table === 'rss_items') {
      ...
      events.push({ 
        ...
        original_post_time: event.original_post_time || event.timestamp || new Date().toISOString(),
        updated_at: event.updated_at || new Date().toISOString(),
        last_edited_by: event.last_edited_by || null
      });
      writeJsonFile(LOCAL_EVENTS_FILE, events);
    }
    return event;
  ...
  try {
    ...
    if (event.source_table === 'rss_items') {
      ...
      await sql`
        INSERT INTO sigint_events (id, title, category, severity, location, lat, lon, timestamp, url, details, created_at, edited, original_post_time, updated_at, last_edited_by)
        VALUES (${event.id}, ${event.title}, ${event.category}, ${event.severity}, ${event.location},
          ...
          ${event.original_post_time || event.timestamp}, ${event.updated_at || new Date().toISOString()}, ${event.last_edited_by || null})
        ON CONFLICT (id) DO NOTHING
      `;
    }
