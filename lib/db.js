...
export async function getEvents(timespan = '24h') {
  ...
  try {
    ...
    let rows = [];
    if (timespan === 'today') {
      const res = await sql`SELECT * FROM sigint_events WHERE edited = TRUE OR (timestamp >= CURRENT_DATE - INTERVAL '7 days') ORDER BY timestamp DESC`;
      rows = res.rows;
    } else {
      const res = await sql`SELECT * FROM sigint_events WHERE edited = TRUE OR (timestamp >= NOW() - CAST(${interval} AS INTERVAL) - INTERVAL '7 days') ORDER BY timestamp DESC LIMIT 500`;
      rows = res.rows;
    }
    ...
    if (updatedEvent) {
      try {
        ...
        for (const ev of sigRes.rows) {
          ...
          const matchUrl = normUrl && getNormalizedUrl(ev.url) === normUrl;
          if (matchTitle || matchUrl) {
            const latVal = parseFloat(fields.lat) || 0.0;
            const lonVal = parseFloat(fields.lon) || 0.0;
            await sql`
    ...
    if (updatedItem) {
      try {
        ...
        for (const item of rssRes.rows) {
          ...
          const matchUrl = normUrl && getNormalizedUrl(item.url) === normUrl;
          if (matchTitle || matchUrl) {
            const latVal = parseFloat(fields.latitude) !== undefined ? parseFloat(fields.latitude) : item.latitude;
            const lonVal = parseFloat(fields.longitude) !== undefined ? parseFloat(fields.longitude) : item.longitude;
            await sql`
              UPDATE rss_items 
              SET latitude = ${latVal}, longitude = ${lonVal}, location = ${fields.location || item.location}, 
                  category = ${fields.category || item.category}, severity = ${parseInt(fields.severity) || item.severity},
        ...
        for (const ev of eventsRes.rows) {
          ...
          const matchUrl = normUrl && getNormalizedUrl(ev.url) === normUrl;
          if (matchTitle || matchUrl) {
            const latVal = parseFloat(fields.latitude) !== undefined ? parseFloat(fields.latitude) : ev.lat;
            const lonVal = parseFloat(fields.longitude) !== undefined ? parseFloat(fields.longitude) : ev.lon;
            await sql`
              UPDATE sigint_events 
              SET lat = ${latVal}, lon = ${lonVal}, location = ${fields.location || ev.location},
                  category = ${fields.category || ev.category}, severity = ${parseInt(fields.severity) || ev.severity},
...
async function cascadeArchive(cleanId, archivedBy) {
  let targetTitle = '';
  let targetUrl = '';
  ...
  if (!process.env.POSTGRES_URL) {
    ...
    const filteredRss = [];
    for (const r of rss) {
      const matchTitle = getNormalizedTitle(r.title) === normTitle;
  ...
  try {
    ...
    if (resSig.rows.length > 0) {
      targetTitle = resSig.rows[0].title;
      targetUrl = resSig.rows[0].url;
    } else {
      const resRss = await sql`SELECT title, url FROM rss_items WHERE id = ${cleanId}`;
      if (resRss.rows.length > 0) {
        targetTitle = resRss.rows[0].title;
        targetUrl = resRss.rows[0].url;
      }
    }
...
export async function archiveEvent(eventId, archivedBy) {
  const cleanEventId = String(eventId).replace(/^db-/, '').replace(/^rss-/, '');
  return cascadeArchive(cleanEventId, archivedBy);
}

export async function archiveRssItem(itemId, archivedBy) {
  const cleanItemId = String(itemId).replace(/^db-/, '').replace(/^rss-/, '');
  return cascadeArchive(cleanItemId, archivedBy);
}

async function cascadeDeletePermanently(cleanId) {
  let targetTitle = '';
  let targetUrl = '';
...
export async function deleteEventPermanently(eventId) {
  const cleanEventId = String(eventId).replace(/^db-/, '').replace(/^rss-/, '');
  return cascadeDeletePermanently(cleanEventId);
}

export async function deleteRssItemPermanently(itemId) {
  const cleanItemId = String(itemId).replace(/^db-/, '').replace(/^rss-/, '');
  return cascadeDeletePermanently(cleanItemId);
}
