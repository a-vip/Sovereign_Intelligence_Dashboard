...
async function generateCommitMessage() {
  try {
    ...
    const message = await askDeepSeek(prompt, systemPrompt);
...
async function summarizeRecentChanges() {
  try {
    ...
    const brief = await askDeepSeek(prompt, systemPrompt);
...
if (mode === 'commit') {
  ...
  console.log('Usage: node gitDeepSeek.js [commit|brief]');
}
